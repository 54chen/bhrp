...
const contractAddress = '0x9DE6d3c670e6123De3E16981f29CbA1c44B4f7E0';//'0x671f8921295a4f582f9296245502508422FBEB92';//'0x1D162cc9d194204Bed89D95e8d7eB64Dfb6ca307';
const contractABI = abi.abi;

const HMTAddress = '0x1F6c8a7c8908410f24Cad4803811cEdaa46eECBE';
const HMTABI = HMT.abi;

const HGTAddress = '0x3B95617f6eF4B3BD9042Cf6168bE449838783203';
...
export async function withdraw(amount, id) {
  try {
    const { ethereum } = window;
    ...
    if (ethereum) {
      ...
      const paidTxn = await wavePortalContract.withdraw(amount); //TODO ID check
...
export async function agree(amount, address, id) {
  try {
    ...
    if (ethereum) {
      ...
      let tokenURI = "https://bhrp.vercel.app/api/property/"+id;

      const wavePortalContract = new ethers.Contract(contractAddress, contractABI, signer);
      const paidTxn = await wavePortalContract.agree(amount, address, id, tokenURI);
      console.log("Mining...", paidTxn.hash);
...
export async function awardItem(id) {
  try {
    const { ethereum } = window;
    ...
    if (ethereum) {
      ...
      const wavePortalContract = new ethers.Contract(contractAddress, contractABI, signer);
      //awardItem(address player, string memory tokenURI, uint256 newItemId)
      const paidTxn = await wavePortalContract.awardItem(tokenURI, id)
      console.log("Mining...", paidTxn.hash);
